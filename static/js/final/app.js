...
angular.module("mainModule").controller('alloyController', function($scope, instagramService, twitterService, sidebarService, alloy) {
  console.log("INSIDE alloyController"); // THIS WILL BE USED TO SWIPE BETWEEN SOCIAL PLATFORMS
  $scope.onSwipeLeft = function(ev) { alert('You swiped left!!'); };
  $scope.onSwipeRight = function(ev) { alert('You swiped right!!'); }; // THE INSTAGRAM SERVICE MTHOD IS USED TO SNATCH T
  // HE URL FROM BROWSER WIDNOW AND PARSE IT FOR LATER USE
  $scope.windowInfoWithToken = instagramService.getWindowInfo(); // THIS GRABS THE INDEX OF THE MENU ITEM SELECTED
  $scope.getTwitterDropDownNumberIndex = sidebarService.getTwitterDropDownNumberIndex(); // THIS IS THE GRAND SEARCH METHOD - PINGING THE THREE MAJOR API'S I'M WORKING WITH
  $scope.getIgandTwitterApiData = function() {
    if ($scope.inputSearchTweetsAndInstagramQuery.length >= 1) { // THE PARSED TOKEN PLUS THE QUERY IS SENT TO THE SERIVCE WHICH THENS MAKES A REST CALL TO THE BACK END
      // A CALL BACK IS PASSED TO RECEIVE THE DATA SENT BACK FROM THE BACK END - WE SET THE RESPONCE TO INSTAGRAMDATA
      instagramService.tapInstaExtended($scope.windowInfoWithToken, $scope.inputSearchTweetsAndInstagramQuery, function(response) { // console.info(response.data);
      ...
      alloy.getSpotifyDATA({ q: $scope.inputSearchTweetsAndInstagramQuery, count: 20 }, function(response) { // SPOTIFYDATA IS USED IN THE VIEW TO PRESENT DATA
      ...
      sidebarService.getTwitterData($scope.inputSearchTweetsAndInstagramQuery, function(response) { // for user timelines   var tweets = response;
        // for tags var tweets = response.data;
        // THIS IS WHERE SOME OF THE SIDEBAR DROPDOWN LOGIC LIVES.
        // SWITCH CASE - FOR MORE OPTIONS
        var tweets = response;
        if (sidebarService.getTwitterDropDownNumberIndex() === 1) { tweets = response; } else { tweets = response.data; }
        console.log(tweets); // TWITTER DATA USED TO PAINT THE CANVAS
        $scope.twitterData = tweets;
      });
    }
  }; // THIS WAS USED TO DO A GET REQUEST WHEN AN INSTAGRAM HASHTAG LINK WAS CLICKED
  $scope.tagQuery = function(instaQuery) { // TOKEN + QUERY + CALLBACK = RESPONSE OBJECT
    instagramService.tapInstaExtended($scope.windowInfoWithToken, instaQuery, function(response) { $scope.instagramData = response.data; });
  }; // USED TO DO A GET REQUEST WHEN A TWITTER HASHTAG LINK WAS PRESSED
  $scope.twitterTagQuery = function(twitterTagSearch) {
    sidebarService.setTwitterDropDownNumberIndex(2); // TWITTER DOESN'T ALWAYS NEED A TOKEN TO AUTHENTICATE - THEY JUST NEED THE QUERY
    sidebarService.getTwitterData(twitterTagSearch, function(response) { // for user timelines   var tweets = response;
      // for tags var tweets = response.data;
      var tweets = response; // USED TWICE FOR TESTING
  ...
  $scope.hideThisDiv = false; // CUSTOM CSS TO HIDE THE SCROLL BEFORE BEFORE THE DASHBOARD IS VISISBLE
  $scope.customOverFlow = function(value) {
    if ($scope.hideThisDiv) {
      return { "overflow": 'auto' }; } else {
      return { "overflow": 'hidden' }; } }; // DEPRECATED - USED AS REFERENCE
  $scope.changeThisSearchTweets = function() { alloy.getTwitterAndInstagramDataByTags({ q: $scope.inputSearchTweetsAndInstagramQuery, count: 25 }, function(response) {
      var tweets = response.data;
      console.log(tweets);
      $scope.twitterData = tweets; }); }; // THE METHODS ABOVE ARE TRIGGERED WHEN THE STATE OF THE VIEW IS CHANGES
  ...
  alloy.getSpotify($scope.windowInfoWithToken, function(response) { $scope.spotifyData = response.data;
  ...
  $scope.doThisNow = function() {
    instagramService.tapInsta($scope.windowInfoWithToken, function(response) {
      ...
      window.alert(response.data.access_token); // debugger;
      // IF THE TOKEN IS NOT UNDEFINED - ASSIGN THE VALUE TO A SCOPE VARIABLE
      // IF NOT DO NOT SHOW THE DASHBOARD
      if (!response.data.access_token == undefined) { $scope.instagramDataWithToken = response.data.access_token; } else { $scope.hideThisDiv = true; }
  ...
  $scope.doThisNow();
  if (windowLocation.indexOf("code") != -1) { $scope.hideThisDiv = true;
    console.log("YES");
    console.log("YES");
    console.log("YES");
    console.log("YES");
    ...
    console.log("YES");
    console.log("YES");
    console.log("YES"); } else { $scope.hideThisDiv = true;
    console.log("NO");
    console.log("NO");
    console.log("NO");
    console.log("NO");
...
angular.module("mainModule").controller('disasterController', function($scope, disasterService) {
  ...
  disasterService.getDangerJSON(function(response) {
    ...
    $scope.dangerData = data; // for (var key in data) {
  ...
  disasterService.getDangerImage('catoosa', function(response) {
    ...
    console.log(data);
    $scope.dangerDataImage = data; // for (var key in data) {
    //     console.log("key: " + key[0]);
    //     console.log("data: " + key + " = " + data[key]);
    //  }
  });
...
angular.module("mainModule").controller('disasterNavController', function($scope, disasterNavService) { console.log("INSIDE disasterNavController"); });
'use strict';
console.log("OUTSIDE fpController");
angular.module("mainModule").controller('fpController', function($scope, fpService) {
  var _console;
  var _marked = [simpleGenerator, getNextID].map(regeneratorRuntime.mark);
  ...
  function loadImg(url) {
    return new Promise(function(resolve, reject) {
      ...
      image.onerror = function() {
        var message = "ERROR: " + url;
        reject(new Error(message)); };
      image.src = url; }); }
  Promise.all([loadImg('http://www.deltbf.com/wp-content/uploads/2015/12/slide1-image-tablet.png')]).then(function(images) { images.forEach(function(img) {
      return addImg(img.src); }); }).catch(function(err0r) { // 
  });
  var addImg = function addImg(src) {
    var imgElement = document.createElement('img');
    imgElement.src = src;
    document.body.appendChild(imgElement); }; // loadImagePromised('http://www.deltbf.com/wp-content/uploads/2015/12/slide1-image-tablet.png')
  // .then(img) => {
  //   var imgElement = document.createElement('img');
  ...
  // DEC 9
  // DEC 8TH 
  var cirlceArea = function cirlceArea(r) {
    var PI = 3.14;
    return PI * (r * r); };
  console.log(cirlceArea(55)); // TEMPLATE LITERAL
  var stringASAName = 'Brendan';
  console.log("My name is " + stringASAName + " Carr");
  console.log("My name is " + stringASAName + " Carr");
  var addNumbers = function addNumbers(a, b, c) { console.log(a + b + c); };
  var numZ = [3, 4, 5];
  console.log(numZ[0], numZ[1], numZ[2]);
  (_console = console).log.apply(_console, numZ); // SPREAD OPERATOR
  var foreignFoods = ['beef patty', 'lo mein'];
  var foods = ['cranberry', 'chicken', 'bbq'];
  console.log(foods);
  var foods = ['cranberry'].concat(foreignFoods, ['chicken', 'bbq']);
  console.log(foods); // CLASSES
  var Person = function() {
    function Person(name, age, weight) { _classCallCheck(this, Person);
      this.name = name;
      this.age = age;
      this.weight = weight; }
    _createClass(Person, [{ key: "displayName", value: function displayName() { console.log(this.name); } }, { key: "displayAge", value: function displayAge() { console.log(this.age); } }, { key: "displayWeight", value: function displayWeight() { console.log(this.weight); } }]);
    return Person; }();
  var triston = new Person('Triston', 10, 110);
  triston.displayName();
  triston.displayAge();
  triston.displayWeight(); // INHERITANCE
  var Programmer = function(_Person) { _inherits(Programmer, _Person);

    function Programmer(name, age, weight, language) { _classCallCheck(this, Programmer);
      var _this = _possibleConstructorReturn(this, (Programmer.__proto__ || Object.getPrototypeOf(Programmer)).call(this, name, age, weight));
      _this.language = language;
      return _this; }
    _createClass(Programmer, [{ key: "displayLanguage", value: function displayLanguage() { console.log(this.language); } }]);
    return Programmer; }(Person);
  var brendan = new Programmer('Brendan', 29, 150, "JS");
  brendan.displayLanguage(); // GENERATORS
    ...
    return regeneratorRuntime.wrap(function simpleGenerator$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
          case "end":
            return _context.stop(); } } }, _marked[0], this); };
  var sample = simpleGenerator();
  ...
  function getNextID() {
    var id;
    return regeneratorRuntime.wrap(function getNextID$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            id = 0;
          case 1:
            if (!(id < 3)) { _context2.next = 6;
              break; }
            _context2.next = 4;
            return id++;
          case 4:
          ...
          case 6:
          case "end":
            return _context2.stop(); } } }, _marked[1], this); };
  var createUser = getNextID();
  console.log(createUser.next().value);
  console.log(createUser.next().value);
  console.log(createUser.next().value);
  console.log(createUser.next().value); // DEC 8TH 
  /*
...
angular.module("mainModule").controller('instagramController', function($scope, instagramService, twitterService, alloy) {
  console.log("INSIDE instagramController"); // I USED THE ALLOY CONTROLLER FOR MOST LOGIC
});
'use strict';
console.log("outer portfolioController");
angular.module("shell").controller('portfolioController', function($scope, dataService) { console.log("portfolioController initialized!"); });
'use strict';
console.log("OUTSIDE sidebarController");
angular.module("mainModule").controller('sidebarController', function($scope, sidebarService) { console.log("INSIDE sidebarController");
  $scope.getTwitterDropDownNumber = 1;
  $scope.getTwitterDropDownText = function(menuOptionNumber) { sidebarService.setTwitterDropDownNumberIndex(menuOptionNumber);
...
'use strict';
console.log("OUTSIDE sidebarController");
angular.module("mainModule").controller('spotifyController', function($scope, instagramService, twitterService, alloy) { console.log("INSIDE sidebarController"); });
'use strict';
console.log("INSIDE twitterController");
angular.module("mainModule").controller('twitterController', function($scope, instagramService, twitterService, alloy) {
  console.log("OUTSIDE twitterController"); // I USED THE ALLOY CONTROLLER FOR MOST LOGIC
});
